...
export default class Pinyin {
  ...
  convert (hans, options) {
    ...
    for (let i = 0, firstCharCode, words, l = hans.length; i < l; i++) {
      ...
      if (this._dict[firstCharCode]) {
        ...
        pys.push(this.singlePinyin(words, options))
      } else {
        nohans += words
      }
    }
  ...
  singlePinyin (han, options) {
    if (typeof han !== 'string') {
      return []
    }
    if (han.length !== 1) {
      return this.singlePinyin(han.charAt(0), options)
    }
  ...
  static toFixed (pinyin, style) {
    ...
    switch (style) {
      ...
      case PINYIN_STYLE.FIRST_LETTER:
        ...
        if (PHONETIC_SYMBOL.hasOwnProperty(firstLetter)) {
          firstLetter = PHONETIC_SYMBOL[firstLetter].charAt(0)
        }
        return firstLetter

      case PINYIN_STYLE.NORMAL:
        return pinyin.replace(RE_PHONETIC_SYMBOL, function ($0, $1Phonetic) {
          return PHONETIC_SYMBOL[$1Phonetic].replace(RE_TONE2, '$1')
        })

      case PINYIN_STYLE.TO3NE:
        return pinyin.replace(RE_PHONETIC_SYMBOL, function ($0, $1Phonetic) {
          return PHONETIC_SYMBOL[$1Phonetic]
        })
