...
export default {
  ...
  methods: {
    ...
    INPUT_CASCADER (option) {
      ...
      const { label, fieldName, labelWidth, labelOptions, itemList = [], style = {}, placeholder = '请选择...', readonly, disabled, change = () => {} } = option
      const { titles = [] } = option
      return (
        <el-form-item key={fieldName} label={label} labelWidth={labelWidth} prop={fieldName}>
          {labelOptions && <span slot="label">{this.createFormItemLabel(labelOptions)}</span>}
    ...
    RANGE_TIME_SELECT (option) {
      ...
      const { label, fieldName, labelWidth, labelOptions, valueFormat = 'HH:mm', options = {}, disabled, change = () => {} } = option
      const { startTime = '00:00', endTime = '23:45', stepTime = '00:15' } = options
    ...
    UPLOAD_IMG (option) {
      const { form } = this
      const { label, fieldName, labelWidth, labelOptions, upload = {}, style = {}, disabled, change = () => {}, iswatermark, fileSize } = option
      return (
        <el-form-item key={fieldName} label={label} labelWidth={labelWidth} prop={fieldName}>
          {labelOptions && <span slot="label">{this.createFormItemLabel(labelOptions)}</span>}
          <UploadCropper
    ...
    UPLOAD_ALIOSS (option) {
      const { form } = this
      // const { label, fieldName, labelWidth, labelOptions, upload = {}, style = {}, initialValue = [], disabled, change = () => {}, error = () => {} } = option
      const { label, fieldName, labelWidth, labelOptions, upload = {}, initialValue = [], change = () => {} } = option
